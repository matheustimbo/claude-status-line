...
A Portuguese (BR) status line for [Claude Code](https://claude.com/claude-code) showing the active model, current effort level, context window usage, and rate limits (5-hour session and 7-day weekly), with colors and time until reset.
...
Claude Code pipes a JSON payload via stdin to the status line command on every render, containing `model`, `effort`, `context_window`, and `rate_limits`. The script parses it with `jq` and prints a formatted line with ANSI colors.
